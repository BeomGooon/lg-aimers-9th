...
def main():
    ...
    # ---- 3. 시드 앙상블 학습 (Training Multiple Seeds) ----
    ...
    for seed in SEEDS:
        print(f"\n==================================")
        print(f"Training Model with Random Seed: {seed}")
        print(f"==================================")
        ...
        print(f"\n[Seed {seed}] Best iteration found: {best_iteration}")
        
        val_preds = val_model.predict_proba(X_val)[:, 1]
        val_preds_list.append(val_preds)
        
        print(f"\n[Seed {seed}] Starting Final Retraining on ENTIRE dataset (2019~2024)...")
    ...
    # ---- 4. 앙상블(평균) 검증 점수 출력 ----
    print("\n==================================")
    print("ENSEMBLE 2024 Season Validation Results")
    ensemble_preds = np.mean(val_preds_list, axis=0)
    
    val_auc = roc_auc_score(y_val, ensemble_preds)
    val_loss = log_loss(y_val, ensemble_preds)
    
    brier_score = np.mean((ensemble_preds - y_val)**2)
    r = np.mean(y_val)

...
def preprocess_data_func(df):
    ...
    # 트랙맨 투구 폼 분산 지표 통합 (L2 거리)
    if 'prev_yr_rel_side_std' in df.columns and 'prev_yr_rel_height_std' in df.columns:
        # 좌우, 상하 릴리스 포인트 분산의 L2 Norm (유클리디안 거리) 계산
        df['prev_yr_rel_point_l2_std'] = np.sqrt(df['prev_yr_rel_side_std']**2 + df['prev_yr_rel_height_std']**2)
        # 익스텐션은 유지하고, 합쳐진 릴리스 포인트 2개 변수는 삭제
        df.drop(columns=['prev_yr_rel_side_std', 'prev_yr_rel_height_std'], inplace=True)
        
    # 트랙맨 피처 추가
    trackman_cols = [c for c in df.columns if c.startswith('prev_yr_')]
    use_features += trackman_cols
...
def main():
    ...
    # ---- 트랙맨 데이터 병합 ----
    df = preprocess_trackman.append_trackman_features(df, 'data/trackman_history.csv', 'pitcher_mapping.csv')
    ...
    # 모델 저장 (테스트용)
    model_path = os.path.join(MODEL_DIR, "catboost_single_trackman.cbm")
